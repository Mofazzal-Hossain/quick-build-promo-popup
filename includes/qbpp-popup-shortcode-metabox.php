...
?>
    <div class="qbpp-popup-shortcode-box">
        ...
        <button id="qbppCopyButton" class="qbpp-popup-copy" type="button"><?php esc_html_e('Copy', 'quick-build-promo-popup'); ?></button>
        <div id="qbppPopupMessage" class="qbpp-popup-message"></div>
    </div>
    <?php
}
...
function qbpp_display_popup($atts)
{
    ...
    // Fetch meta data
    $qbpp_active = get_post_meta($post->ID, '_qbpp_popup_active', true);
    $qbpp_thumbnail_id = absint(get_post_meta($post->ID, '_qbpp_image_id', true));
    $qbpp_url = sanitize_url(get_post_meta($post->ID, '_qbpp_popup_url', true));
    $qbpp_popup_content_heading = get_post_meta($post->ID, '_qbpp_popup_content_heading', true);
    $qbpp_popup_content_desc = get_post_meta($post->ID, '_qbpp_popup_content_desc', true);
    $qbpp_custom_content_active = get_post_meta($post->ID, '_qbpp_custom_content_active', true);
    $qbpp_popup_custom_content = get_post_meta($post->ID, '_qbpp_popup_custom_content', true);
    ...
    $qbpp_display = absint(get_post_meta($post->ID, '_qbpp_popup_display', true));
    $qbpp_auto_hide = get_post_meta($post->ID, '_qbpp_popup_auto_hide', true);
    $qbpp_thumbnail_size = sanitize_text_field(get_post_meta($post->ID, '_qbpp_popup_size', true));
    ...
    $qbpp_hide_delay = $qbpp_auto_hide ? absint(get_post_meta($post->ID, '_qbpp_popup_hide_delay', true)) : '';
    ...
    $has_custom_content = $qbpp_custom_content_active && !empty($qbpp_popup_custom_content);

    // Image class
    $image_class = ($has_custom_content || $qbpp_popup_content_heading) ? 'qbpp_exist_content' : '';
                    ...
                    <div class="modal-body">
                        <?php $qbpp_modal_body_content = '
                            <a href="' . esc_url($qbpp_url) . '" aria-label="' . esc_url($qbpp_url) . '" title="' . esc_url($qbpp_url) . '">
                                <img class="qbpp-popup-image ' . esc_attr($image_class) . '" src="' . esc_url($qbpp_thumbnail[0]) . '"  alt="">
                            </a>
                            <div class="qbpp-popup-content">
                                <h3 class="qbpp-popup-heading align-' . $qbpp_heading_align . '">' . esc_html($qbpp_popup_content_heading) . '</h3>
                                <p class="qbpp-popup-desc align-' . $qbpp_desc_align . '">' . wp_kses_post($qbpp_popup_content_desc) . '</p>
                            </div>
                        ';
                        if($has_custom_content){
                            $qbpp_modal_body_content .= '<div class="qbpp-popup-custom-content">' . wp_kses_post($qbpp_popup_custom_content) . '</div>';
                        }
